...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background font-inter">
      {/* Navbar */}
      <nav className={`fixed top-0 w-full bg-background/80 backdrop-blur-lg border-b border-border z-50 transition-all duration-300 ${hasScrolled ? 'shadow-md' : ''}`}>
        <div className="container mx-auto px-6 h-20 md:h-24 flex justify-between items-center">
          ...
          >
            <span className="text-foreground">OPPER</span>
              ...
              <button 
                ...
                className={`capitalize font-medium relative after:content-[''] after:absolute after:left-0 after:bottom-[-4px] after:w-full after:h-[2px] after:bg-primary-red after:transition-transform after:duration-300 after:origin-center ${
                  activeSection === item 
                    ...
                    : 'text-muted-foreground hover:text-primary-red after:scale-x-0 hover:after:scale-x-100'
                }`}
              >
                {item}
              </button>
        ...
        {/* Mobile Menu Panel */}
        {isMenuOpen && (
          <div className="md:hidden bg-background/95 backdrop-blur-lg py-2 border-t border-border flex flex-col items-center">
            {navItems.map((item) => (
              <button 
                key={item} 
                onClick={() => scrollToSection(item)} 
      ...
      {/* Hero Section */}
      <section id="home" className="pt-28 md:pt-40 pb-20 bg-background">
        <motion.div 
        ...
        >
          ...
          <motion.div variants={itemVariants} className="mt-8 flex flex-wrap gap-x-8 gap-y-4 justify-center text-muted-foreground">
        ...
        <div className="container mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="text-center lg:text-left">
              ...
              <p className="text-muted-foreground mb-4">A OpperX nasceu com a missão de transformar ideias em resultados reais. Mesmo sendo uma agência jovem, carregamos a energia da inovação e a experiência de profissionais apaixonados por marketing digital.</p>
              <p className="text-muted-foreground mb-4">Nosso diferencial está no "X": o fator extra que entregamos em cada projeto — criatividade, estratégia e performance. Trabalhamos lado a lado com nossos clientes para construir marcas fortes, gerar impacto e criar conexões autênticas com o público.</p>
              <p className="text-muted-foreground">Na OpperX, cada cliente é mais que um projeto. É uma parceria. 🚀</p>
            </div>
            <div className="grid sm:grid-cols-3 gap-6">
              <Card className="text-center shadow-sm"><CardContent className="p-6"><p className="text-4xl font-bold text-primary-red mb-2">10+</p><p className="text-muted-foreground">Anos de Experiência</p></CardContent></Card>
              <Card className="text-center shadow-sm"><CardContent className="p-6"><p className="text-4xl font-bold text-primary-red mb-2">500+</p><p className="text-muted-foreground">Clientes Atendidos</p></CardContent></Card>
              <Card className="text-center shadow-sm"><CardContent className="p-6"><p className="text-4xl font-bold text-primary-red mb-2">95%</p><p className="text-muted-foreground">de Satisfação</p></CardContent></Card>
            </div>
          </div>
        </div>
      </AnimatedSection>
      ...
      {/* Serviços */}
      <AnimatedSection id="servicos" className="py-20 bg-background">
        <div className="container mx-auto px-6">
            ...
            {servicesData.map(service => (
              <motion.div key={service.title} variants={itemVariants} className="h-full">
                <Card className="text-center flex flex-col h-full hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                  <CardHeader>
                    <div className="mx-auto w-16 h-16 bg-primary-red/10 rounded-full flex items-center justify-center mb-4">
          ...
          <div className="text-center mt-16">
            ...
            <p className="text-muted-foreground mb-6 max-w-2xl mx-auto">Fale com a gente hoje mesmo e descubra como podemos impulsionar seus resultados.</p>
            <Button 
      ...
      {/* Carousel Section */}
      <AnimatedSection id="portfolio" className="py-20 bg-secondary">
      ...
      <AnimatedSection id="contato" className="py-20">
        <div className="container mx-auto px-6">
          <div className="text-center mb-12">
            ...
            <div className="mt-4 inline-flex items-center gap-3 bg-secondary-yellow/10 dark:bg-secondary-yellow/20 text-foreground text-lg font-semibold px-6 py-3 rounded-full border border-secondary-yellow/30 dark:border-secondary-yellow/40 shadow-sm">
              ...
              <CardContent className="flex-grow flex flex-col justify-center space-y-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <a href={`tel:+${settings?.contact_phone?.replace(/\D/g, '')}`} className="text-muted-foreground hover:text-primary-red">{loading ? 'Carregando...' : formatPhoneNumberForDisplay(settings?.contact_phone || '')}</a>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <a href="mailto:opperstoreofc@gmail.com" className="text-muted-foreground hover:text-primary-red">opperstoreofc@gmail.com</a>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <a href={settings?.instagram_url} target="_blank" rel="noopener noreferrer" aria-label={`Abrir Instagram ${settings?.instagram}`} className="text-muted-foreground hover:text-primary-red">{loading ? 'Carregando...' : settings?.instagram}</a>
                  </div>
                </div>
              </CardContent>
            </Card>
